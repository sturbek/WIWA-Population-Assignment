...
contemp_assignments <- spread(data=rep_indiv_estsC,repunit,rep_pofz) %>% dplyr::select(indiv,mixture_collection,State,AK2Alberta,CoastalCA,Eastern,PacNorthwest,RockyMtn,Sierra,Latitude,Longitude) %>% mutate(Highest_PofZ=max(AK2Alberta,CoastalCA,Eastern,PacNorthwest,RockyMtn,Sierra))
...
# Spread the data so each individual is a row and the probability of assignment is spread across columns representing each genetic cluster
histor_assignments <- spread(data=rep_indiv_estsC,repunit,rep_pofz) %>% dplyr::select(indiv,mixture_collection,State,AK2Alberta,CoastalCA,Eastern,PacNorthwest,RockyMtn,Sierra,Latitude,Longitude) %>% mutate(Highest_PofZ=max(AK2Alberta,CoastalCA,Eastern,PacNorthwest,RockyMtn,Sierra))
...
histor_assignment_certainty <- histor_assignments %>% 
  ...
  mutate(sum=AK2Alberta+CoastalCA+Eastern+PacNorthwest+RockyMtn+Sierra) %>% 
  mutate(assignment=if_else(AK2Alberta==1,"AK2Alberta",if_else(CoastalCA==1,"CoastalCA",if_else(Eastern==1,"Eastern",if_else(PacNorthwest==1,"PacNorthwest",if_else(RockyMtn==1,"RockyMtn",if_else(Sierra==1,"Sierra","uncertain"))))))) %>% 
  mutate(PofZ=if_else(AK2Alberta==1,"AK2Alberta",if_else(CoastalCA==1,"CoastalCA",if_else(Eastern==1,"Eastern",if_else(PacNorthwest==1,"PacNorthwest",if_else(RockyMtn==1,"RockyMtn",if_else(Sierra==1,"Sierra","uncertain")))))))
